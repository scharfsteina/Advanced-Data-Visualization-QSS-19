...
df_sw <- df %>% 
  filter(str_detect(airline, "Southwest"))

df_sw %>% 
  ggplot(aes(x = month)) +
  geom_area(aes(y = ontime), alpha = .4, fill = "#344cb4") +
  geom_area(aes(y = delayed), alpha = .5, fill = "#344cb4") +
  geom_area(aes(y = cancelled), alpha = .7, fill = "#344cb4") +
  geom_line(aes(y = ontime), alpha = .3, color= "black") +
  geom_line(aes(y = delayed), alpha = .3, color= "black") +
  geom_line(aes(y = cancelled), alpha = .3, color= "black") +
  scale_x_date(date_breaks = "3 months", date_labels = "%b '%y")+
  theme_minimal() +
  labs(x = "Month", y = "Number of Flights",
       title = "Flights Ontime, Delayed, and Cancelled for Southwest Airlines") +
...
df_delta %>% 
  ggplot(aes(x = month)) +
  geom_area(aes(y = ontime), alpha = .4, fill = "#e41c34") +
  geom_area(aes(y = delayed), alpha = .5, fill = "#e41c34") +
  geom_area(aes(y = cancelled), alpha = .7, fill = "#e41c34") +
  geom_line(aes(y = ontime), alpha = .3, color= "black") +
  geom_line(aes(y = delayed), alpha = .3, color= "black") +
  geom_line(aes(y = cancelled), alpha = .3, color= "black") +
  scale_x_date(date_breaks = "3 months", date_labels = "%b '%y")+
  theme_minimal() +
  labs(x = "Month", y = "Number of Flights",
       title = "Flights Ontime, Delayed, and Cancelled for Delta Airlines") +
  theme(legend.position = "none",
        panel.grid.major = element_blank(), panel.grid.minor = element_blank()) 

df_delta %>% 
  pivot_longer(5:7, names_to = "status", values_to = "count") %>%
  mutate(status = factor(status, levels = c("ontime","delayed", "cancelled"))) %>% 
  #uncount(count)
  ggplot(aes(x = month, group = status, fill = status, y = count)) +
  #geom_line(aes(color = status))+
  geom_col()+
  scale_x_date(date_breaks = "3 months", date_labels = "%b '%y")+
  # scale_fill_manual(values = c("#1C5D99", "#5BC0BE","#FB5607"),
...
p <- df_delta %>% 
  ggplot(aes(x = month)) +
  #geom_col(aes(y = ontime), alpha = .3, fill = "white") +
  geom_col(aes(y = flights), alpha = .8, fill = "#E3132C") +
  geom_col(aes(y = delayed), alpha = 1, fill = "#9B1631") +
  geom_col(aes(y = cancelled), alpha = .6, fill = "black") +
  # ggimage::geom_image(size = 0.15, by="height", x = as.Date("2022-06-01"),
  #                                                 y = 70000, image = "data/airplane.png") +
  scale_x_date(date_breaks = "6 months", date_labels = "%b %Y", limits = c(as.Date("2018-01-01"),as.Date("2022-11-01")))+
  labs(x = "\nMonth", y = "Number of Flights\n",
       title = "Delta Airlines Flights Ontime, Delayed, and Cancelled") +
  theme_minimal() +
  theme(legend.position = "none",
        panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        ...
        title = element_text(size = 16)) #Avenir Next
  
ggdraw(p) +
  draw_image(magick::image_read("data/airplane2.png"),
                    scale = .15, x = .95, y = .95, hjust = 1, vjust = 1, halign = 1, valign = 1)
